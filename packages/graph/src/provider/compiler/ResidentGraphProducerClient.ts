...
/** Restartable NDJSON client for a compiler graph producer. */
export class ResidentGraphProducerClient {
  ...
  public constructor(options: ResidentGraphProducerClient.IOptions) {
  ...
  public produce(
    ...
    if (signal?.aborted === true) {
      return Promise.reject(cancelled(this.provider, this.label));
    }
    ...
    return new Promise<undefined>((resolve, reject) => {
      const pending: Pending = {
        ...
        timer: setTimeout(() => {
          this.failChild(
            child,
            new Error(
              `${this.provider}: ${this.label} request timed out after ${String(this.requestTimeoutMs)} ms${stderrSuffix(child)}`,
            ),
          );
        }, this.requestTimeoutMs),
        signal,
      ...
      if (signal !== undefined) {
        pending.abort = () =>
          this.failChild(child, cancelled(this.provider, this.label));
  ...
  private ensureChild(): Child {
    ...
    const invocation = spawnableCommand.append(
      ...
      [this.serverCommand, "--cwd", this.root],
    );
    ...
    process.on("error", (error) =>
      this.failChild(
        child,
        new Error(`${this.provider}: ${this.label} server failed: ${error.message}`),
      ),
    );
    /* c8 ignore stop */
    process.stdin.on("error", (error) =>
      this.failChild(
        child,
        new Error(
          `${this.provider}: ${this.label} server stdin failed: ${error.message}${stderrSuffix(child)}`,
        ),
      ),
    );
    process.on("exit", (code, signal) =>
      this.failChild(
        child,
        new Error(
          `${this.provider}: ${this.label} server exited (${String(signal ?? code)})${stderrSuffix(child)}`,
        ),
      ),
    );
    return child;
    ...
    for (;;) {
      ...
      try {
        ...
        this.failChild(
          child,
          new Error(
            `${this.provider}: invalid ${this.label} server response: ${asError(error).message}`,
          ),
        );
        return;
      }
      ...
      try {
        response = parseResponse(value, this.provider, this.label);
      } catch (error) {
      ...
      if (pending === undefined) {
        this.failChild(
          child,
          new Error(
            `${this.provider}: unexpected ${this.label} response id ${String(response.id)}`,
          ),
        );
        return;
      }
      ...
      if (pending.child !== child) {
        this.failChild(
          child,
          new Error(
            `${this.provider}: ${this.label} response crossed producer processes`,
          ),
        );
        return;
      }
  ...
  private append(child: Child, chunk: string): boolean {
    ...
    if (child.responseBytes > this.maxResponseBytes) {
      this.failChild(
        child,
        new Error(
          `${this.provider}: ${this.label} response exceeded the ${String(this.maxResponseBytes)} byte limit`,
        ),
      );
      return false;
    }
...
function parseResponse(value: unknown, provider: string, label: string): IResponse {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error(`${provider}: ${label} response must be an object`);
  }
  const row = value as Record<string, unknown>;
  if (!Number.isSafeInteger(row.id) || row.protocolVersion !== PROTOCOL_VERSION) {
    throw new Error(`${provider}: invalid ${label} response identity`);
  }
  ...
  throw new Error(`${provider}: invalid ${label} response result`);
}

function cancelled(provider: string, label: string): Error {
  const error = new Error(`${provider}: ${label} request was aborted`);
...
export namespace ResidentGraphProducerClient {
