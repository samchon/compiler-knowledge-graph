...
const INPUT = path.resolve(
  process.env.SAMCHON_GRAPH_BENCH_INPUT ?? path.join(ROOT, "results", "graph.json"),
);
const OUT_DIR = path.resolve(
  process.env.SAMCHON_GRAPH_BENCH_RENDER_OUT ?? path.join(ROOT, "results"),
);
  ...
  return [...byRepo.entries()]
    .map(([repo, repoCells]) => {
      const byTool = new Map(
        repoCells.map((cell) => [cell.tool ?? "samchon-graph", cell]),
      );
...
function medianAnswerMs(cells, project, tool) {
  const durations = cells
    .filter(
      (cell) => cell.repo === project && (cell.tool ?? "samchon-graph") === tool,
    )
    .flatMap((cell) => [
