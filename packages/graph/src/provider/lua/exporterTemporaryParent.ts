 ...
 * namespace. The same sibling replaces a user-configured temp directory inside
 * the project, where the exporter would become an input to its own index. A
 * project that is itself the namespace root has no safe location and is
 * declined rather than receiving generated index inputs.
 */
export function exporterTemporaryParent(
  ...
  const temporaryIsOutside =
    !paths.isAbsolute(relative) &&
    relative.split(paths.sep)[0] === "..";
  return temporaryIsOutside ? resolvedTemporary : sibling;
}
