...
/**
 * Turn one committed `scip-java --kotlin-graph-output` artifact into a validated
 ...
 * incremental build rewrites only the sources kotlinc recompiled. So this
 ...
 */
export class KotlinGraphSnapshotAdapter {
  ...
  public apply(
    ...
    // Only what moved. A Gradle build that recompiled one source
...
function helloOf(
  ...
  return {
    ...
    languages: ["kotlin"],
    authority: "compiler",
    supportedFacts: [...KOTLIN_GRAPH_FACTS],
    capabilities: [...CAPABILITIES],
  };
}

/**
 * The Kotlin compiler versions that ran the plugin, as a set rather than a pick.
 *
 * Every shard states the `kotlin.version` of the compiler that produced it, and
 ...
 */
function compilerVersionOf(raw: IKotlinGraphSnapshot): string {
 ...
 */
function generationOf(raw: IKotlinGraphSnapshot): string {
  return digest(
...
function universeOf(raw: IKotlinGraphSnapshot): string {
  return digest(
...
function targetShards(
  root: string,
  target: IKotlinGraphSnapshot.ITarget,
  universe: string,
 ...
 * target is a producer defect whether or not the two records agree: kotlinc
  ...
  for (const shard of target.shards) {
    for (const node of shard.nodes) {
      if (declared.has(node.symbol)) {
        throw new Error(
          `Kotlin graph: symbol ${node.symbol} is declared twice in target ${target.name}`,
        );
      }
...
function sourceShard(
  root: string,
  target: IKotlinGraphSnapshot.ITarget,
  shard: IKotlinGraphSnapshot.IShard,
...
function metadataShard(
  ...
  const coordinate = `bundled:///kotlin/target/${digest([target.name])}`;
  return {
    key: `kotlin-target:${digest([target.name, target.universe])}`,
    target: target.name,
    languages: ["kotlin"],
    nodes,
    edges: [],
 ...
 * not all know the same amount. A reference kotlinc could attribute carries the
    ...
    for (const edge of shard.edges) {
      for (const symbol of [edge.from, edge.to]) {
        ...
        if (
          ...
          throw new Error(
            `Kotlin graph: external symbol ${symbol} is named two ways in target ${target.name}`,
          );
        }
      }
    }
...
function namingOf(edge: IKotlinGraphSnapshot.IEdge): INaming {
  const name =
...
/**
 * Kotlin producer names are already separated from structural signatures.
 */
...
function adaptNode(
  root: string,
  target: IKotlinGraphSnapshot.ITarget,
  node: IKotlinGraphSnapshot.INode,
): ISamchonGraphNode {
...
function adaptEvidence(
  root: string,
  evidence: IKotlinGraphSnapshot.IEvidence,
): ISamchonGraphEvidence {
  return {
...
function assertSnapshot(
  value: unknown,
  root: string,
): IKotlinGraphSnapshot {
  if (!isRecord(value)) {
    throw new Error("Kotlin graph: the graph artifact is not an object");
  }
  const raw = value as unknown as IKotlinGraphSnapshot;
  if (raw.schemaVersion !== IKotlinGraphSnapshot.SCHEMA_VERSION) {
    throw new Error(
      `Kotlin graph: unsupported artifact schema ${String(raw.schemaVersion)}; this adapter reads ${String(IKotlinGraphSnapshot.SCHEMA_VERSION)}`,
    );
  }
  if (!isRecord(raw.producer)) {
    throw new Error("Kotlin graph: the artifact names no producer");
  }
  if (raw.producer.name !== KOTLIN_GRAPH_PRODUCER) {
    throw new Error(
      `Kotlin graph: foreign producer ${String(raw.producer.name)}`,
    );
  }
  if (raw.producer.protocolVersion !== IKotlinGraphSnapshot.PROTOCOL_VERSION) {
    throw new Error(
      `Kotlin graph: unsupported producer protocol ${String(raw.producer.protocolVersion)}`,
    );
  }
  if (typeof raw.producer.version !== "string" || raw.producer.version === "") {
    throw new Error("Kotlin graph: the producer states no version");
  }
  ...
  if (
    ...
    throw new Error("Kotlin graph: the producer states no capability block");
  }
  ...
  if (!capabilities.atomicGenerations) {
    throw new Error(
      "Kotlin graph: the producer does not commit atomic generations",
    );
  }
  if (!capabilities.incremental) {
    throw new Error(
      "Kotlin graph: the producer does not preserve incremental generations",
    );
  }
  if (!capabilities.diagnostics) {
    throw new Error(
      "Kotlin graph: the producer does not publish compiler diagnostics",
    );
  }
  if (typeof raw.projectRoot !== "string" || raw.projectRoot === "") {
    throw new Error("Kotlin graph: the artifact names no project root");
  }
  if (!samePath(raw.projectRoot, root)) {
    throw new Error(
      `Kotlin graph: the artifact was produced for ${raw.projectRoot}, not ${root}`,
    );
  }
  if (!Array.isArray(raw.targets) || raw.targets.length === 0) {
    throw new Error("Kotlin graph: the artifact committed no target");
  }
...
function assertTarget(
  ...
  if (
    ...
    throw new Error("Kotlin graph: malformed committed target");
  }
  if (names.has(target.name)) {
    throw new Error(`Kotlin graph: duplicate committed target ${target.name}`);
  }
  ...
  if (
    ...
    throw new Error(
      `Kotlin graph: target ${target.name} has an incomplete coverage matrix`,
    );
  }
  ...
  for (const family of GRAPH_EDGE_KINDS) {
    if (
      !KOTLIN_GRAPH_FACTS.includes(family) &&
      target.coverage[family] !== "unsupported"
    ) {
      throw new Error(
        `Kotlin graph: target ${target.name} claims ${family}, which this route does not prove`,
      );
    }
  }
  for (const shard of target.shards) {
...
function assertShard(
  ...
  if (
    !isRecord(shard) ||
    shard.schemaVersion !== IKotlinGraphSnapshot.SCHEMA_VERSION ||
    shard.language !== "kotlin" ||
    ...
    // it, one against the normalized path a graph node carries. `./a/B.kotlin`
    // and `a/B.kotlin` are the same file and different strings, so a producer
    ...
    // later by a fence that cannot say it was the producer's doing. Every Kotlin
    ...
    throw new Error(
      `Kotlin graph: malformed shard in target ${target.name}`,
    );
  }
  const key = `${target.name}\0${shard.source}`;
  if (sources.has(key)) {
    throw new Error(
      `Kotlin graph: source ${shard.source} is committed twice in target ${target.name}`,
    );
  }
  sources.add(key);
  ...
  for (const node of shard.nodes) {
    if (
      ...
      throw new Error(
        `Kotlin graph: malformed declaration in ${shard.source}`,
      );
    }
    if (symbols.has(node.symbol)) {
      throw new Error(
        `Kotlin graph: duplicate declaration ${node.symbol} in ${shard.source}`,
      );
    }
  ...
  for (const edge of shard.edges) {
    if (
      ...
      !KOTLIN_GRAPH_FACTS.includes(edge.kind as GraphEdgeKind) ||
      ...
      throw new Error(`Kotlin graph: malformed edge in ${shard.source}`);
    }
  ...
  for (const site of shard.unresolved) {
    if (
      ...
      throw new Error(
        `Kotlin graph: malformed unresolved site in ${shard.source}`,
      );
    }
  ...
  for (const diagnostic of shard.diagnostics) {
    if (
      ...
      throw new Error(
        "Kotlin graph: malformed diagnostic in " + shard.source,
      );
    }
    assertEvidence(diagnostic.evidence, shard.source);
  }
}
...
function assertEvidence(
  ...
  if (
    ...
    throw new Error(`Kotlin graph: malformed evidence in ${source}`);
  }
}
...
function shardKey(target: string, source: string): string {
  return `kotlin-shard:${digest([target, source])}`;
}
