 ...
 *
 * Every released `scip-java` answers `index`, and none of them writes this graph:
 * `--kotlin-graph-output` arrived with the kotlinc plugin. A route that resolved on the
 * command name alone would run a whole Gradle build and only then
 ...
 */
export const test_kotlinc_graph_declines_a_launcher_without_graph_output =
  async (): Promise<void> => {
    const root = GraphPaths.createTempDirectory("samchon-graph-kotlinc-select-");
    fs.writeFileSync(
    ...
    const shim = (name: string, flags: readonly string[]): string =>
      script(
        name,
        `"${process.execPath}" "${GraphPaths.fakeKotlinGraph}" ${flags.join(" ")} ${windows ? "%*" : '"$@"'}`,
      );
    // Every JDK since 9 answers `--version` on standard output, which is
    // where the shared toolchain probe reads.
    const jdk = script("java", "echo openjdk 21.0.12 2026-10-21");

    const select = (launcher: string, options = {}) =>
      selectGraphProviders(root, ["kotlin"], options, {
        ...process.env,
        SAMCHON_GRAPH_KOTLINC_GRAPH: launcher,
        SAMCHON_GRAPH_JAVA_TOOLCHAIN: jdk,
      });

    const legacy = select(shim("legacy", ["--fake-legacy-launcher"]));
    TestValidator.predicate(
      "a launcher without --kotlin-graph-output is not selected",
      legacy.candidates.every(
        (candidate) => candidate.provider.name !== KOTLIN_GRAPH_PROVIDER,
      ),
    );
    TestValidator.predicate(
      ...
      legacy.warnings.some(
        (warning) =>
          warning.includes(KOTLIN_GRAPH_PROVIDER) &&
          warning.includes("compiler provider") &&
          warning.includes("was not found"),
      ),
    );

    const legacyServer = select(
      shim("legacy-server", ["--fake-legacy-server"]),
    );
    TestValidator.predicate(
      "a launcher without the resident protocol is not selected",
      legacyServer.candidates.every(
        (candidate) => candidate.provider.name !== KOTLIN_GRAPH_PROVIDER,
      ),
    );
    ...
    TestValidator.predicate(
      ...
      selected.candidates.some(
        (candidate) =>
          candidate.provider.name === KOTLIN_GRAPH_PROVIDER &&
          candidate.provider.authority === "compiler" &&
          candidate.languages.join() === "kotlin",
      ),
    );
    ...
    // recompiles against a different JDK and a launcher upgrade can
    // move the shard schema, so both are identity rather than decoration.
    const environment = {
      ...process.env,
      SAMCHON_GRAPH_KOTLINC_GRAPH: current,
      SAMCHON_GRAPH_JAVA_TOOLCHAIN: jdk,
    };
    const configuration = kotlinGraphProvider.configuration?.(root, environment);
    TestValidator.predicate(
      "the build universe names the JDK and the launcher that will run",
      configuration?.length === 2 &&
        configuration[0]!.startsWith("java=") &&
        configuration[0]!.includes("21.0.12") &&
    ...
    const command = kotlinGraphProvider.resolve(root, environment);
    TestValidator.predicate(
      "the registered route resolves its launcher",
      command !== undefined,
    );
    if (command === undefined) {
      throw new Error("kotlinc-graph: the fixture launcher did not resolve");
    }

    const mavenRoot = GraphPaths.createTempDirectory(
      "samchon-graph-kotlinc-maven-",
    );
    fs.writeFileSync(
      path.join(mavenRoot, "pom.xml"),
    ...
    TestValidator.predicate(
      "the Gradle-only compiler route declines a Maven project",
      unsupportedBuild.candidates.every(
        (candidate) => candidate.provider.name !== KOTLIN_GRAPH_PROVIDER,
      ),
    );
    ...
    try {
      const session = kotlinGraphProvider.open({
        root,
        command,
        languages: ["kotlin"],
        options: { cwd: root },
      ...
      try {
        ...
        TestValidator.predicate(
          ...
          generation.mode === "initial" &&
            generation.snapshot.provenance.provider === KOTLIN_GRAPH_PROVIDER &&
            generation.snapshot.provenance.authority === "compiler" &&
            generation.snapshot.provenance.compilerVersion === "2.3.20" &&
      ...
      TestValidator.predicate(
        ...
        (dump.provenance ?? []).some(
          ...
          dump.nodes.some(
            (node) => node.file === "src/main/kotlin/com/Example.kt",
          ),
      );
      ...
      TestValidator.predicate(
        ...
        (edited.provenance ?? []).some(
          (row) => row.provider === KOTLIN_GRAPH_PROVIDER,
        ),
      );
    } finally {
