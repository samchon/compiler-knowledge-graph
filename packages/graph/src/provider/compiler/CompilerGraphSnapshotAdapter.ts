...
import { ICompilerGraphSnapshot } from "./ICompilerGraphSnapshot";
 ...
 *
 * Diagnostics are explicit because the producer exports FIR diagnostics with
 * each source shard rather than asking the consumer to scrape build output.
 */
const CAPABILITIES = [
  "coverage",
  "diagnostics",
...
/**
 * Turn one committed compiler artifact into a validated Graph Snapshot
 * Protocol generation.
 *
 ...
 * incremental build rewrites only the sources its compiler recompiled. So this
 ...
 */
export class CompilerGraphSnapshotAdapter {
  ...
  public constructor(
    ...
    private readonly contract: CompilerGraphSnapshotAdapter.IContract,
  ) {
  ...
  public apply(
    ...
    const raw = assertSnapshot(value, this.root, this.contract);
    const rawTargets = [...raw.targets].sort((left, right) =>
    ...
    const hello = helloOf(raw, this.contract);
    const identity = canonical(hello);
    const targets = rawTargets.map((target) => target.name);
    ...
    // Only what moved. A build that recompiled one source
...
export namespace CompilerGraphSnapshotAdapter {
  export interface IContract {
    ...
    identitySalt?: (target: ICompilerGraphSnapshot.ITarget) => string;
    validateSnapshot?: (snapshot: ICompilerGraphSnapshot) => void;
    validateTarget?: (
      target: ICompilerGraphSnapshot.ITarget,
    ) => void;
    validateShard?: (
      shard: ICompilerGraphSnapshot.IShard,
      target: ICompilerGraphSnapshot.ITarget,
      root: string,
    ) => void;
  }
...
function helloOf(
  raw: ICompilerGraphSnapshot,
  contract: CompilerGraphSnapshotAdapter.IContract,
...
/**
 * The compiler versions that ran the plugin, as a set rather than a pick.
 *
 * Every shard states the compiler version that produced it, and one build can
 * legitimately run multiple toolchains across its targets. The
 ...
 */
function compilerVersionOf(raw: ICompilerGraphSnapshot): string {
 ...
 */
function generationOf(raw: ICompilerGraphSnapshot): string {
  return digest(
    [...raw.targets]
      .sort((left, right) => compareText(left.name, right.name))
      .map((target) => [target.name, target.generation, target.universe]),
  );
}

/** The same composition over the universes alone. */
function universeOf(raw: ICompilerGraphSnapshot): string {
  return digest(
    [...raw.targets]
      .sort((left, right) => compareText(left.name, right.name))
      .map((target) => [target.name, target.universe]),
  );
}
...
function targetShards(
  ...
  const declared = declaredNodes(root, target, contract);
  const externals = externalNodes(target, declared, contract);
  ...
  shards.push(metadataShard(target, externals, universe, located, contract));
  return shards;
}
 ...
 * target is a producer defect whether or not the two records agree: a compiler
  ...
  for (const shard of target.shards) {
    for (const node of shard.nodes) {
      if (declared.has(node.symbol)) {
        throw new Error(
          `${contract.label}: symbol ${node.symbol} is declared twice in target ${target.name}`,
        );
      }
      declared.set(node.symbol, adaptNode(root, target, node, contract));
    }
  }
  return declared;
}
...
function sourceShard(
  ...
  return {
    key: shardKey(contract, target.name, shard.source),
    target: target.name,
    languages: [contract.language],
    nodes,
    edges,
    diagnostics: shard.diagnostics.map(
      (diagnostic): ISamchonGraphDiagnostic => ({
        file: graphFile(root, diagnostic.evidence.file),
        line: diagnostic.evidence.startLine,
        column: diagnostic.evidence.startColumn,
        code: contract.diagnosticCode,
        message: diagnostic.message,
        severity: diagnostic.severity as ISamchonGraphDiagnostic["severity"],
      }),
    ),
    coverage: [],
...
function metadataShard(
  ...
  const coordinate = `bundled:///${contract.language}/target/${digest([target.name])}`;
  return {
    key: `${contract.shardKeyPrefix}-target:${digest([target.name])}:${target.universe}`,
    target: target.name,
    languages: [contract.language],
    nodes,
    edges: [],
 ...
 * not all know the same amount. A compiler-attributed reference carries the
    ...
    for (const edge of shard.edges) {
      for (const symbol of [edge.from, edge.to]) {
        ...
        if (
          ...
          throw new Error(
            `${contract.label}: external symbol ${symbol} is named two ways in target ${target.name}`,
          );
        }
      }
    }
  ...
  for (const [symbol, naming] of described) {
    ...
    externals.set(symbol, {
      id: semanticGraphNodeId(
        ...
        native: {
          key: identityKey(contract, target, symbol),
          stability: "semantic",
        },
...
function namingOf(edge: ICompilerGraphSnapshot.IEdge): INaming {
  const name =
...
/**
 * Producer names are already separated from structural signatures.
 */
...
function adaptNode(
  root: string,
  target: ICompilerGraphSnapshot.ITarget,
  node: ICompilerGraphSnapshot.INode,
  contract: CompilerGraphSnapshotAdapter.IContract,
): ISamchonGraphNode {
...
function adaptEvidence(
  root: string,
  evidence: ICompilerGraphSnapshot.IEvidence,
): ISamchonGraphEvidence {
  return {
...
function assertSnapshot(
  ...
  root: string,
  contract: CompilerGraphSnapshotAdapter.IContract,
): ICompilerGraphSnapshot {
  if (!isRecord(value)) {
    throw new Error(`${contract.label}: the graph artifact is not an object`);
  }
  const raw = value as unknown as ICompilerGraphSnapshot;
  if (raw.schemaVersion !== contract.schemaVersion) {
    throw new Error(
      `${contract.label}: unsupported artifact schema ${String(raw.schemaVersion)}; this adapter reads ${String(contract.schemaVersion)}`,
    );
  }
  if (!isRecord(raw.producer)) {
    throw new Error(`${contract.label}: the artifact names no producer`);
  }
  if (raw.producer.name !== contract.producer) {
    throw new Error(
      `${contract.label}: foreign producer ${String(raw.producer.name)}`,
    );
  }
  if (raw.producer.protocolVersion !== contract.protocolVersion) {
    throw new Error(
      `${contract.label}: unsupported producer protocol ${String(raw.producer.protocolVersion)}`,
    );
  }
  if (typeof raw.producer.version !== "string" || raw.producer.version === "") {
    throw new Error(`${contract.label}: the producer states no version`);
  }
  ...
  if (
    ...
    throw new Error(`${contract.label}: the producer states no capability block`);
  }
  ...
  if (!capabilities.atomicGenerations) {
    throw new Error(
      `${contract.label}: the producer does not commit atomic generations`,
    );
  }
  if (!capabilities.incremental) {
    throw new Error(
      `${contract.label}: the producer does not preserve incremental generations`,
    );
  }
  if (!capabilities.diagnostics) {
    throw new Error(
      `${contract.label}: the producer does not publish compiler diagnostics`,
    );
  }
  contract.validateSnapshot?.(raw);
  if (typeof raw.projectRoot !== "string" || raw.projectRoot === "") {
    throw new Error(`${contract.label}: the artifact names no project root`);
  }
  if (!samePath(raw.projectRoot, root)) {
    throw new Error(
      `${contract.label}: the artifact was produced for ${raw.projectRoot}, not ${root}`,
    );
  }
  if (!Array.isArray(raw.targets) || raw.targets.length === 0) {
    throw new Error(`${contract.label}: the artifact committed no target`);
  }
  const names = new Set<string>();
  const sources = new Set<string>();
  for (const target of raw.targets) {
    assertTarget(target, names, sources, root, contract);
  }
  return raw;
}
...
function assertTarget(
  ...
  if (
    ...
    throw new Error(`${contract.label}: malformed committed target`);
  }
  contract.validateTarget?.(target);
  if (names.has(target.name)) {
    throw new Error(`${contract.label}: duplicate committed target ${target.name}`);
  }
  ...
  if (
    ...
    throw new Error(
      `${contract.label}: target ${target.name} has an incomplete coverage matrix`,
    );
  }
  ...
  for (const family of GRAPH_EDGE_KINDS) {
    if (
      !contract.facts.includes(family) &&
      target.coverage[family] !== "unsupported"
    ) {
      throw new Error(
        `${contract.label}: target ${target.name} claims ${family}, which this route does not prove`,
      );
    }
  }
  for (const shard of target.shards) {
    assertShard(shard, target, sources, root, contract);
  }
}
...
function assertShard(
  ...
  if (
    !isRecord(shard) ||
    shard.schemaVersion !== contract.schemaVersion ||
    shard.language !== contract.language ||
    ...
    // it, one against the normalized path a graph node carries. `./a/File.ext`
    // and `a/File.ext` are the same file and different strings, so a producer
    ...
    // later by a fence that cannot say it was the producer's doing. Every
    // compiler unit has an on-disk identity, including a generated one;
    ...
    throw new Error(
      `${contract.label}: malformed shard in target ${target.name}`,
    );
  }
  contract.validateShard?.(shard, target, root);
  const key = `${target.name}\0${shard.source}`;
  if (sources.has(key)) {
    throw new Error(
      `${contract.label}: source ${shard.source} is committed twice in target ${target.name}`,
    );
  }
  sources.add(key);
  ...
  for (const node of shard.nodes) {
    if (
      ...
      throw new Error(
        `${contract.label}: malformed declaration in ${shard.source}`,
      );
    }
    if (symbols.has(node.symbol)) {
      throw new Error(
        `${contract.label}: duplicate declaration ${node.symbol} in ${shard.source}`,
      );
    }
    symbols.add(node.symbol);
    assertEvidence(node.evidence, shard.source, contract);
  }
  for (const edge of shard.edges) {
    if (
      !isRecord(edge) ||
      ...
      !contract.facts.includes(edge.kind as GraphEdgeKind) ||
      ...
      throw new Error(`${contract.label}: malformed edge in ${shard.source}`);
    }
    assertEvidence(edge.evidence, shard.source, contract);
  }
  for (const site of shard.unresolved) {
    if (
      ...
      throw new Error(
        `${contract.label}: malformed unresolved site in ${shard.source}`,
      );
    }
    assertEvidence(site.evidence, shard.source, contract);
  }
  for (const diagnostic of shard.diagnostics) {
    if (
      ...
      throw new Error(
        `${contract.label}: malformed diagnostic in ${shard.source}`,
      );
    }
    assertEvidence(diagnostic.evidence, shard.source, contract);
  }
}
...
function assertEvidence(
  ...
  if (
    ...
    throw new Error(`${contract.label}: malformed evidence in ${source}`);
  }
}
...
function shardKey(
  ...
  return `${contract.shardKeyPrefix}-shard:${digest([target, source])}`;
}
...
function identityKey(
  ...
  const salt = contract.identitySalt?.(target);
  return salt === undefined ? symbol : `${salt}\0${symbol}`;
}
