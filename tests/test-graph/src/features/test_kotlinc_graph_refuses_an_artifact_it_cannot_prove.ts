...
import { IKotlinGraphSnapshot, KotlinGraphSnapshotAdapter } from "@samchon/graph";
 ...
 */
export const test_kotlinc_graph_refuses_an_artifact_it_cannot_prove =
  (): void => {
    const root = GraphPaths.createTempDirectory("samchon-graph-kotlinc-wire-");
    fs.mkdirSync(path.join(root, "src", "main", "kotlin", "com"), {
      recursive: true,
    });
    fs.writeFileSync(
    ...
    ) as IKotlinGraphSnapshot;

    // The baseline itself must publish, or every refusal below would pass for
    // the wrong reason.
    const accepting = new KotlinGraphSnapshotAdapter(root);
    ...
    const rejects = (
      label: string,
      mutate: (value: IKotlinGraphSnapshot) => void,
    ): void => {
    ...
    rejects("a producer that cannot preserve incremental generations", (value) => {
      value.producer.capabilities.incremental = false;
    });
    rejects("a producer that cannot publish diagnostics", (value) => {
      value.producer.capabilities.diagnostics = false;
    });
    ...
    rejects("an artifact with no producer block", (value) => {
      (value as { producer?: unknown }).producer = "not-a-producer-block";
    });
    ...
    rejects("a shard of another language", (value) => {
      value.targets[0]!.shards[0]!.language = "java";
    });
    ...
    // carries. `./a/B.kt` and `a/B.kt` are one file and two strings, so a
    ...
    for (const [label, source] of [
      ["a source spelled with a leading dot segment", "./src/main/kotlin/com/Example.kt"],
      ["a source spelled with an interior dot segment", "src/main/./kotlin/com/Example.kt"],
      ["a source that climbs out of the project", "../Example.kt"],
      ["a source spelled with Windows separators", "src\\main\\kotlin\\com\\Example.kt"],
      ["a source given as an absolute POSIX path", "/src/main/kotlin/com/Example.kt"],
      ["a source given as an absolute Windows path", "C:/src/Example.kt"],
      ["a source carrying a NUL", "src/main/kotlin/com/Exa\0mple.kt"],
      ["a source with an empty segment", "src//Example.kt"],
      ["a source that is only a dot segment", "."],
      ["a source spelled as a directory", "src/main/kotlin/com/"],
    ] as const) {
      rejects(label, (value) => {
    ...
    // empty reading is the producer failing to read `kotlin.version`, not a
    // build without one.
    ...
    rejects("a declaration attributed to another source", (value) => {
      value.targets[0]!.shards[0]!.nodes[0]!.file = "../Outside.kt";
    });
    ...
    rejects("a declaration with no compiler origin", (value) => {
      value.targets[0]!.shards[0]!.nodes[0]!.origin = "";
    });
    ...
    rejects("diagnostics that are not a list", (value) => {
      (value.targets[0]!.shards[0] as { diagnostics?: unknown }).diagnostics = {};
    });
    rejects("a diagnostic severity outside the shared vocabulary", (value) => {
      const shard = value.targets[0]!.shards.find(
        (entry) => entry.diagnostics.length > 0,
      )!;
      shard.diagnostics[0]!.severity = "fatal";
    });
    rejects("a diagnostic with no message", (value) => {
      const shard = value.targets[0]!.shards.find(
        (entry) => entry.diagnostics.length > 0,
      )!;
      shard.diagnostics[0]!.message = "";
    });
    rejects("evidence with no file", (value) => {
      unresolvedShard(value).unresolved[0]!.evidence.file = "";
    });
    rejects("evidence attributed to another source", (value) => {
      unresolvedShard(value).unresolved[0]!.evidence.file = "../Outside.kt";
    });
    ...
    rejects("one external symbol described two ways", (value) => {
      ...
      shard.edges.push({
        ...
        targetQualifiedName: "kotlin.lang.Object.somethingElse",
      });
    });
  };
...
function unresolvedShard(
  value: IKotlinGraphSnapshot,
): IKotlinGraphSnapshot.IShard {
