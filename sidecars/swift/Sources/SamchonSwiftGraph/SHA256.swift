    ...
    for offset in stride(from: 0, to: message.count, by: 64) {
      ...
      for index in 16..<64 {
        let s0 = rotate(words[index - 15], by: 7) ^
          rotate(words[index - 15], by: 18) ^ (words[index - 15] >> 3)
        let s1 = rotate(words[index - 2], by: 17) ^
          rotate(words[index - 2], by: 19) ^ (words[index - 2] >> 10)
      ...
      var h = state[7]
      for index in 0..<64 {
        let sum1 = rotate(e, by: 6) ^ rotate(e, by: 11) ^ rotate(e, by: 25)
        let choose = (e & f) ^ ((~e) & g)
        let temporary1 = h &+ sum1 &+ choose &+ constants[index] &+ words[index]
        let sum0 = rotate(a, by: 2) ^ rotate(a, by: 13) ^ rotate(a, by: 22)
