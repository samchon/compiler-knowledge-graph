...
import { CompilerGraphSnapshotAdapter } from "./CompilerGraphSnapshotAdapter";
import { ResidentGraphProducerClient } from "./ResidentGraphProducerClient";

const DEFAULT_MAX_ARTIFACT_BYTES = 256 * 1024 * 1024;

/**
 * A strict compiler route's session: one resident build connection and one
 * graph generation per changed input universe.
 *
 * The producer is attached to the project's real compile tasks, so a refresh
 * is a normal build. That is the point of the route rather than a limitation
 * of it: the build tool's own incremental state
 ...
 */
export class CompilerGraphSession implements IBulkGraphSession {
  ...
  private readonly adapter: CompilerGraphSnapshotAdapter;
  private readonly validate: (snapshot: IBulkGraphSession.ISnapshot) => void;
  private readonly producer: ResidentGraphProducerClient;
  private readonly batch: BatchGraphSession;

  public constructor(options: CompilerGraphSession.IOptions) {
    ...
    this.producer = new ResidentGraphProducerClient({
      root: options.root,
    ...
    let configuration:
      | ReturnType<NonNullable<CompilerGraphSession.IOptions["configuration"]>>
      | undefined;
    this.batch = new BatchGraphSession({
      ...
      // This session owns the producer process and its build connection. Their
   ...
   * build that printed a message where its graph should be—and builds can
   * print a great deal.
   */
...
export namespace CompilerGraphSession {
  export interface IOptions {
    root: string;
    languages: readonly GraphLanguage[];
    provider: string;
    command: IGraphProvider.ICommand;
    adapter: CompilerGraphSnapshotAdapter;
